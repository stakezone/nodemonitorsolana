...
Additional useful modules for GPU and S.M.A.R.T monitoring are available from the Zabbix site.

### New

Balances are now polled for the identity pubkey with a related trigger for a low amount.
...
As of time of writing, a cluster can sometimes produce inconsistent timestamps that lead to wrong time calculation for the `elapsed` time showing large lags.
